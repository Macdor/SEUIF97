LIBDIR=./bin/
ifeq ($(OS),Windows_NT)
    LIBNAME =$(LIBDIR)seuif97.dll 
	LIBFLAGES=-static-libstdc++ -static-libgcc -static -Wl,--add-stdcall-alias,--output-def=$(LIBDIR)seuif97.def,-out-implib=$(LIBDIR)seuif97.lib
else
	UNAME_S := $(shell uname -s)
    ifeq ($(UNAME_S),Linux)
        LIBNAME =$(LIBDIR)seuif97.so
    endif
endif
